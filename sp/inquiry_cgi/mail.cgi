...
#---------------------------------------------------------------------------------------------------
		...
		foreach $ERR (@err){
			...
			if($ERR ne ""){
				if($ERR eq $NAME && $VALUE eq ""){
					...
					$MES .= "<li>[<strong class=\"caution\"> $ERR </strong>]　の項目が未入力です。</li>\n";
					$mes = $MES;
				}
			}
		}
	...
	# メールアドレスチェック
	unless($MAIL=~/^[\w\-+\.]+\@[\w\-+\.]+$/i) {
		$MES .= "<li>[<strong class=\"caution\"> E-mail </strong>]　の項目が間違っています。<br>正しいメールアドレスを入力してください。</li>\n";
		$mes = $MES;
	}
	if($mes ne ""){
		$mes .="</ul>\n";
...
#---------------------------------------------------------------------------------------------------
		...
		foreach $CK (@NEW){
			...
			$strhtml .= "<dt>" .$NAME. "</dt><dd>"  .$VALUE. "</dd>\n";
